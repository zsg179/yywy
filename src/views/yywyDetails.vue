...
export default {
    ...
    methods:{
        ...
        cal(){
            var bdnd=1;//保单年度
            var ndmAge=0;//被保险人年度末年龄
            var lastValue=0;//上年度末保单账户价值
            var accountValue=0;//账户价值
            //收益档次计算
            var nh,yh;
            ...
            //计算年度末保单账户价值
            for (ndmAge = this.getNdmAge(ndmAge,bdnd); ndmAge <= this.endAge; bdnd=this.getBdnd(bdnd),ndmAge=this.getNdmAge(ndmAge,bdnd)) {
                if (bdnd == 0) {
                    return lastValue = 0;
                }   
                if (this.calAccountValue(lastValue,bdnd,nh, yh,ndmAge) <= 0) {
                    return lastValue = 0;
                }
                lastValue = (lastValue + this.getDJ(bdnd) * 0.98 + yh / 12 * this.getZjYear(ndmAge) * (bdnd <= 3 ? 0.98 : 0.99) - this.njlq) * (1 + nh) - this.bflq;
            }
            this.totalOutNum = lastValue;
            alert("总价值："+lastValue)
        },
        //计算年度末保单账户价值
        // calNdmAccountValue(bdnd,nh, yh){
        //     if (this.bdnd == 0) {
        ...
        // },
        //计算账户价值
        calAccountValue(lastValue,bdnd,nh, yh,ndmAge){
            //追加方式按月计算
            return (lastValue + this.getDJ(bdnd) * 0.98 + (yh / 12 * this.getZjYear(ndmAge) * (bdnd <= 3 ? 0.98 : 0.99))) * (1 + nh) - this.bflq - this.njlq;
        },
        //计算总投入费用
        getTotalInNum(){
            //趸交+累计追缴
        ...
        //计算被保险人年度末年龄
        getNdmAge(ndmAge,bdnd){
            return ndmAge > (this.bxTime-1) ? 0 : (bdnd == 1 ? this.age + 1 : ndmAge + 1);
        },
        //计算保单年度
        getBdnd(bdnd){
            return bdnd > (this.bxTime-1) ? 0 : bdnd + 1;
        },
        //计算每年追加保费,追加方式按月计算
        getZjYear(ndmAge){
            return 12 * ((ndmAge >= this.zjAgeStart && ndmAge <= this.zjAgeEnd) ? this.znInMonth : 0);
        },
        //计算趸交保费
        getDJ(bdnd){
            this.dj = bdnd > 1 ? 0 : this.dj;
            return this.dj;
        },
        pop() {
            this.popupShow = true;
